...
for tokenization_ in MIREX:
    ...
    if tokenization_ == "MMM":
        for tokenization__ in MMM_BASE_TOKENIZATIONS:
            ...
            params__["base_tokenizer"] = tokenization__
            adjust_tok_params_for_tests(tokenization_, params__)
            TOK_PARAMS_ONE_TRACK_MIREX.append((tokenization_, params__))
    else:
        adjust_tok_params_for_tests(tokenization_, params_)
        TOK_PARAMS_ONE_TRACK_MIREX.append((tokenization_, params_))
...
def test_one_track_midi_to_tokens_to_midi(
    ...
    _test_tokenize_only(file_path, tok_params_set, saving_erroneous_files)
...
if __name__ == "__main__":
    print(TOK_PARAMS_ONE_TRACK)
    for i in range(len(MIDI_PATHS_ONE_TRACK)):
        sc, s, t = _test_tokenize_only(MIDI_PATHS_ONE_TRACK[i], TOK_PARAMS_ONE_TRACK_MIREX[0], saving_erroneous_files=False)
        scr, sr, tr = _test_tokenize_only(MIDI_PATHS_ONE_TRACK[i], TOK_PARAMS_ONE_TRACK[0], saving_erroneous_files=False)
        for i in range(min(len(t[0]), len(tr[0]))):
            tok = t[0][i]
            tokr = tr[0][i]