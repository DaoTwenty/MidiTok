...
class MIREX(MusicTokenizer):
    ...
    **Note:** When decoding multiple token sequences (of multiple tracks), i.e. when
    ...
    def __init__(
        ...
        tokenizer_config.num_velocities = 127
        ...
        for ei, event in enumerate(events):
            ...
            if event.time != previous_tick:
                ...
                if event.type_ != "TimeSig" and not event.type_.startswith("ACBar"):
                    ...
                    if event.delta != 0:
                        microtimes = self._get_micro_times(event.delta)
                        for m in microtimes:
                            all_events.append(
                                Event(
                                    type_="Delta",
    ...
    def _get_micro_times(
        ...
        to microtime base value and max microtime depth

        compute delta tfrom event time in ticks and event position index
        ...
        for i in range(self.microtime_max_res - 1, self.microtime_min_res - 1, -1):
            resolution = base ** i
            res_factor =  min(delta_ticks // resolution, base - 1)
            if res_factor >= base:
